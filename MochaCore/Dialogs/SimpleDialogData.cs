        ...
        public SimpleDialogData()
        {
            ...
            catch (MissingMethodException ex)
            {
                throw new InvalidOperationException(
                    $"Cannot create instance of {typeof(T)} because it does not have a constructor which takes one parameter " +
                    $"of type IDialog<{typeof(T)}>. Either add descirbed constructor to your custom type which derives from " +
                    $"{typeof(T).BaseType} class or do not pass null as dataContext for BaseDialogModule constructor.", ex);
            }
        } 
    }
}
